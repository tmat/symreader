...
function InstallToolset {
  if (!(Test-Path $ToolsetBuildProj)) {
   ...
   '<Project Sdk="RoslynTools.RepoToolset"><Target Name="NoOp"/></Project>' | Set-Content $proj
    & $BuildCmd $proj /t:NoOp /m /nologo /clp:None /warnaserror /v:$verbosity /p:NuGetPackageRoot=$NuGetPackageRoot /p:__ExcludeSdkImports=true
  }
}
...
try {
  ...
  if ($GlobalJson.vswhere.version -ne $null) {
    $DotNetRoot = Join-Path $RepoRoot ".dotnet"
    $BuildCmd = Join-Path $DotNetRoot "dotnet.exe msbuild"
  } else if ($GlobalJson.sdk.version -ne $null) {
    $InVSEnvironment = !($env:VS150COMNTOOLS -eq $null) -and (Test-Path $env:VS150COMNTOOLS)

    $vsInstallDir = LocateVisualStudio

    $BuildCmd = Join-Path $vsInstallDir "MSBuild\15.0\Bin\msbuild.exe /nodeReuse:$(!$ci)"
